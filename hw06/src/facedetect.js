...
function notifyDetectedFace() {
  ...
  try {
    fpsCounter.innerHTML =
      1000 / (smoother.reduce((a, b) => a + b, 0) / smoother.length);
  } catch {}
...
function calculateFaceData(face) {
  ...
  let leftEyeCenter = FACEMESH_LEFT_EYE.map(e => lastDetectedFace[e[0]]).reduce(
    (p, c) => {
      return {
        x: p.x + c.x / FACEMESH_LEFT_EYE.length,
        y: p.y + (c.y * scale) / FACEMESH_LEFT_EYE.length,
        z: p.z + (c.z * scale) / FACEMESH_LEFT_EYE.length,
      };
    },
    { x: 0, y: 0, z: 0 }
  );
  let rightEyeCenter = FACEMESH_LEFT_EYE.map(
    e => lastDetectedFace[e[0]]
  ).reduce(
    (p, c) => {
      return {
        x: p.x + c.x / FACEMESH_RIGHT_EYE.length,
        y: p.y + (c.y * scale) / FACEMESH_LEFT_EYE.length,
        z: p.z + (c.z * scale) / FACEMESH_LEFT_EYE.length,
      };
    },
